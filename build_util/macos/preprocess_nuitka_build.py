...
from build_util_macos.shlib_tools import (
    SharedLib,
    change_rpath_to_new_path,
    get_dylib_paths,
    get_so_paths,
)
...
parser.add_argument(
    "base_directory",
    help="fix the rpaths of the dylib/so under base_directory",
    type=str,
)
parser.add_argument(
    "-t",
    "--target_path",
    help=f'alternative path of "{start_path}"',
    type=str,
    required=True,
)
...
internal_shared_paths: List[Path] = get_so_paths(base_dir_path) + get_dylib_paths(
    base_dir_path
)
# 全ての共有ライブラリファイル名のリスト
...
for shared_lib in has_rpath_started_with_loader_path_shared:
    rpaths: Set[Path] = set(
        [rpath for rpath in shared_lib.rpaths if str(rpath).startswith(start_path)]
    )
    ...
    for dylib_path in shared_lib.rpaths:
        ...
        for replace_rpath in rpaths_started_with_loader_path:
            if dylib_path.name == replace_rpath.name:
                change_rpath_to_new_path(
                    replace_rpath, target_path / dylib_path.name, shared_lib.path
                )
